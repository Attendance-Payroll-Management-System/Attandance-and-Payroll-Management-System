...
// Count
$count_sql = "SELECT COUNT(*) as total FROM leave_requests lr JOIN employee e ON lr.employee_id = e.id WHERE $where";
...
foreach ($records as $r) {
    if ($r['status'] == 'Approved') { $total_approved++; $total_days += $r['total_days'] ?? 1; }
...
<head>
    ...
    <title>HNIN AKARI NWE · Leave Report</title>
...
<body x-data="{}" class="bg-slate-50 dark:bg-[#0B1120] text-slate-900 dark:text-white font-sans antialiased min-h-screen flex">
    ...
    <div class="flex-1 flex flex-col min-w-0 main-wrapper">
        <?php
            ...
            $page_subtitle = "View leave requests by department, weekly or monthly.";
            ob_start();
        ?>
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div class="glass-strong rounded-xl p-4 text-center">
                    <p class="text-2xl font-bold text-white"><?php echo $total_records; ?></p>
                    <p class="text-xs text-zinc-500 mt-1">Total Requests</p>
                </div>
                <div class="glass-strong rounded-xl p-4 text-center">
                ...
                <div class="glass-strong rounded-xl p-4 text-center">
                    <p class="text-2xl font-bold text-rose-400"><?php echo $total_rejected; ?></p>
            ...
            <div class="glass-strong rounded-2xl overflow-hidden">
                <div class="p-4 border-b border-white/[0.06] flex items-center justify-between">
                    <h2 class="font-bold text-white"><i class="fa-solid fa-list text-blue-400 mr-2"></i>Leave Records</h2>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-white/[0.06] text-zinc-300">
                            ...
                            <tr class="hover:bg-white/[0.02]">
                                <td class="px-4 py-3">
                                    <div class="font-medium text-white"><?php echo htmlspecialchars($r['name']); ?></div>
                                ...
                                <td class="px-4 py-3"><span class="text-xs font-semibold px-2 py-1 rounded-full bg-blue-500/15 text-blue-400"><?php echo htmlspecialchars($r['leave_type']); ?></span></td>
                                <td class="px-4 py-3"><?php echo $r['leave_duration'] === 'half_day' ? 'Half Day' : 'Full Day'; ?></td>
                                <td class="px-4 py-3 font-mono text-xs"><?php echo date('M d, Y', strtotime($r['start_date'])); ?></td>
                                <td class="px-4 py-3 font-mono text-xs"><?php echo date('M d, Y', strtotime($r['end_date'])); ?></td>
                                <td class="px-4 py-3">
                                    <?php
